alter table public.payout_obligations
  add column financial_document_id uuid references public.financial_documents(id) on delete restrict;

alter table public.payout_obligations
  alter column financial_document_id set not null;

create unique index payout_obligations_financial_document_uidx
  on public.payout_obligations(financial_document_id);

alter table public.payout_payments
  alter column financial_settlement_id set not null;

create unique index payout_payments_financial_settlement_uidx
  on public.payout_payments(financial_settlement_id);
...
begin
  ...
  loop
    ...
    insert into public.financial_documents(
      ...
      functional_currency_code,functional_amount,
      classification_status,counterparty_account_id,status,
      submitted_by,submitted_at,approved_by,approved_at,journal_entry_id,
      ...
      v_functional_currency,v_functional_amount,
      'classified',v_liability,'approved',
      v_calc.requested_by,v_calc.requested_at,v_calc.approved_by,v_calc.approved_at,v_entry,
...
as $$
declare
  v_payment public.payout_payments;
  v_obligation public.payout_obligations;
  v_settlement public.financial_settlements;
  v_unit text;
  v_remaining numeric;
begin
  ...
  if not found or v_payment.version<>p_expected_version then
    return null;
  end if;

  select * into v_obligation
  from public.payout_obligations
  where id=v_payment.payout_obligation_id
  for update;

  if not found then
    raise exception 'Obrigação de repasse não encontrada.';
  end if;

  v_unit:=private.unit_code_for_id(v_obligation.business_unit_id);
  if not private.user_has_permission(p_actor_user_id,'payout.manage',v_unit) then
    raise exception 'Permissão insuficiente para registrar pagamento.';
  end if;
  if v_payment.status<>'draft' then
    raise exception 'Pagamento já consolidado.';
  end if;
  if v_obligation.status not in('open','partially_paid') then
    raise exception 'A obrigação não permite pagamento neste estado.';
  end if;

  select * into v_settlement
  from public.financial_settlements
  where id=v_payment.financial_settlement_id;

  if not found or v_settlement.status<>'posted' then
    raise exception 'Liquidação financeira não está postada.';
  end if;
  if v_settlement.financial_document_id<>v_obligation.financial_document_id then
    raise exception 'A liquidação não pertence ao documento financeiro desta obrigação.';
  end if;
  if v_settlement.original_currency_code<>v_obligation.currency_code
     or v_payment.currency_code<>v_obligation.currency_code then
    raise exception 'Moeda do pagamento ou da liquidação diverge da obrigação.';
  end if;
  if round(v_payment.amount,2)<>round(v_settlement.original_amount,2) then
    raise exception 'O pagamento deve corresponder exatamente ao valor original da liquidação.';
  end if;

  v_remaining:=v_obligation.amount-v_obligation.paid_amount;
  if v_payment.amount>v_remaining then
    raise exception 'Pagamento excede o saldo da obrigação.';
  end if;

  update public.payout_payments
  set status='posted',posted_by=p_actor_user_id,posted_at=now()
  where id=v_payment.id and version=p_expected_version
