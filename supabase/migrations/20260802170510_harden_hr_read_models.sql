create or replace function private.hr_employee_directory_data(p_unit_code text default null)
returns table (
  employee_id uuid,
  display_name text,
...
create or replace function private.hr_employee_sensitive_detail_data(p_employee_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
set search_path=''
as $$
  select * from private.hr_employee_directory_data(p_unit_code);
$$;
...
set search_path=''
as $$
  select private.hr_employee_sensitive_detail_data(p_employee_id);
$$;

revoke all on function private.hr_employee_directory_data(text) from public,anon;
revoke all on function private.hr_employee_sensitive_detail_data(uuid) from public,anon;
grant usage on schema private to authenticated,service_role;
grant execute on function private.hr_employee_directory_data(text) to authenticated,service_role;
grant execute on function private.hr_employee_sensitive_detail_data(uuid) to authenticated,service_role;
