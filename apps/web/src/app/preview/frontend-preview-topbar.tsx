    ...
    <header className="sticky top-0 z-30 border-b bg-card/95 px-3 py-2 backdrop-blur md:px-5">
      <div className="flex min-h-11 flex-wrap items-center gap-2">
        ...
        <div className="hidden min-w-0 flex-1 md:block">
          <p className={isDashboard ? "text-sm font-semibold" : "text-xs font-medium text-muted-foreground"}>
            {isDashboard ? "Dashboard" : "Sistema Central"}
          </p>
          <p className={isDashboard ? "truncate text-xs text-muted-foreground" : "truncate text-sm font-semibold"}>
            {isDashboard
        ...
        {!isDashboard && <Select value={unit} onValueChange={setUnit}><SelectTrigger className="w-[170px]"><SelectValue placeholder="Unidade" /></SelectTrigger><SelectContent><SelectItem value="todas">Todas as unidades</SelectItem><SelectItem value="music-os-360">Music OS 360</SelectItem><SelectItem value="vivendo-da-musica">Vivendo da Música</SelectItem><SelectItem value="corporativo">Corporativo</SelectItem></SelectContent></Select>}
        {!isDashboard && <Select value={period} onValueChange={setPeriod}><SelectTrigger className="w-[155px]"><CalendarDays className="h-4 w-4" /><SelectValue placeholder="Competência" /></SelectTrigger><SelectContent><SelectItem value="2026-08">Agosto / 2026</SelectItem><SelectItem value="2026-07">Julho / 2026</SelectItem><SelectItem value="2026-06">Junho / 2026</SelectItem></SelectContent></Select>}
