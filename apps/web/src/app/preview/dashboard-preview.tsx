...
export function DashboardPreview() {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => !statusOnly || !["inativo", "fechado", "arquivado"].includes((row.at(-1) ?? "").toLowerCase())), [rows, statusOnly]);
  ...
  return <div className="space-y-5">
    ...
    <section className="min-w-0 overflow-hidden rounded-lg border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b p-4 lg:flex-row lg:items-center lg:justify-between"><p className="text-sm text-muted-foreground">{visibleRows.length} registro(s) nesta visão.</p><div className="flex flex-wrap gap-2"><Button variant={filterOpen ? "default" : "outline"} size="sm" onClick={() => setFilterOpen((value) => !value)}><Filter className="h-4 w-4" /> Filtros</Button><Button variant="outline" size="sm" onClick={() => setCompact((value) => !value)}><SlidersHorizontal className="h-4 w-4" /> {compact ? "Confortável" : "Compacta"}</Button></div></div>
      {filterOpen && <div className="flex flex-wrap items-center gap-3 border-b bg-muted/20 px-4 py-3 text-sm"><Button size="sm" variant={statusOnly ? "default" : "outline"} onClick={() => setStatusOnly((value) => !value)}>Somente ativos</Button><Button size="sm" variant="ghost" onClick={() => setStatusOnly(false)}>Limpar filtros</Button></div>}
      <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-sm"><thead className="bg-muted/50"><tr>{COLUMNS.map((column) => <th key={column} className="whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">{column}</th>)}<th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide text-muted-foreground">Ações</th></tr></thead><tbody>{visibleRows.map((row, index) => <tr key={`${row[0]}-${index}`} className="border-t hover:bg-muted/30">{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`} className={`whitespace-nowrap px-4 ${compact ? "py-2" : "py-3"}`}>{cellIndex === row.length - 1 ? <StatusPill status={cell} /> : cell}</td>)}<td className="px-4 py-2 text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" aria-label={`Ações de ${row[0]}`}><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => setNotice(`${row[0]} selecionado para visualização.`)}><Eye className="h-4 w-4" /> Ver</DropdownMenuItem><DropdownMenuItem onSelect={() => setNotice(`${row[0]} selecionado para edição.`)}><Pencil className="h-4 w-4" /> Editar</DropdownMenuItem><DropdownMenuItem onSelect={() => { setRows((current) => current.filter((item) => item !== row)); setNotice(`${row[0]} arquivado na demonstração.`); }}><Trash2 className="h-4 w-4" /> Arquivar</DropdownMenuItem></DropdownMenuContent></DropdownMenu></td></tr>)}</tbody></table></div>
